...
class Fscan(BeamScan):
    ...
    def ttd_judge(self, doa):
        ...
        m = np.round(self.f0/f_inv)
        f_inv = f_inv*m
        t_peak = (f_inv - (self.f0-self.Kr*self.Tp/2))/(self.Kr)
        if np.sum(m != m[0]) != 0 or np.min(t_peak) < 0 or np.max(t_peak) > self.Tp:
            return False
        return True
    ...
    def calulate_doaTx(self, doa):
        ...
        interval = np.abs(1/(self.N*self.Kr*t_inv))
    ...
    def get_ttd(self, doa):
        ttd_value = self.ttd
        init_range = self.Tp
        for ttd in np.linspace(0, 4e-9, 40000):
            self.ttd = ttd
            if self.ttd_judge(doa) == False:
                continue
            peak, _, _, _ = self.calulate_doaTx(doa)
            swath = np.abs(np.max(peak)-np.min(peak))
            if np.abs(swath - self.Tp) < init_range:
                init_range = np.abs(swath - self.Tp) 
                ttd_value = ttd
        print("ttd: ", ttd_value)
        return ttd_value
...
def fscan_ka_estimate():
    ...
    nesz_fscan = fscan_sim.nesz(doa)
    nesz_strip = strip_sim.nesz(doa)
    plt.figure()
    plt.plot(np.rad2deg(doa), nesz_fscan, label="fscan")
    ...
    plt.figure()
    plt.plot(np.rad2deg(doa), rasr_fscan, label="fscan")
